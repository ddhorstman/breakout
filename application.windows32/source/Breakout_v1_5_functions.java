...
public class Breakout_v1_5_functions extends PApplet {
    ...
    if (x + dx + diam/2 >= left_corner && x + dx -diam/2 <= right_corner) {            //check ball's x-coordinate
      ...
      if (dx>=base_speed*-1.5f) {
        if (right_corner - x >= 90) {                                            //bounce off of corners in the opposite direction
          ...
          if (abs(dy)<=base_speed*1.5f)dy -= base_speed/20;
        }
      }
      if (dx<=base_speed*1.5f) {
        if (x - left_corner >= 90) {
...
public void runSmartAI() {
  ...
  if (dx!=0) { 
    ...
    rebound = false;
    if (number_of_boxes >= 0.2f*nx*ny+1) {                           //run medium AI protocol for many boxes left
      if (abs(dx)/abs(dy)>4) {
        if (dx<0)paddle_target = 120;
        if (dx>0)paddle_target = 20;
      }      //correct overly horizontal movement
      else {
        if (abs(dx)<0.25f*base_speed) {
          if (dx>0)paddle_target = 115;
          if (dx<0)paddle_target = 20;
        }
      } //correct overly vertical movement
    }
    if (number_of_boxes<0.2f*nx*ny+1&&number_of_boxes>=2) {                     //target a specific box if few are left
...
public void wallCollisions() {
  if (x >= width -diam/2) {
    dx=-abs(dx); 
    if (dx <=-0.75f*base_speed)dx += 0.01f*base_speed;
  }
  if (x<= diam/2) { 
    dx = abs(dx);
    if (dx >= 0.75f*base_speed)dx -=0.01f*base_speed;
  ...
  if (y<= diam/2) { 
    dy = abs(dy); 
    if (dy >= 0.75f*base_speed)dy -= 0.01f*base_speed;
  }
}
...
public void drawHeadsUpDisplay() {
  textSize(32); 
  fill(255);     
...
public void launchBall() {
  ...
  if (AI_delay == 0||input_mode!="AI") {
    dx=base_speed*random(0.5f, 1.1f);
    dy=base_speed;
  }
}
...
public void populateBoxes() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Breakout_v1_5_functions" };
    if (passedArgs != null) {
